...
for (run in strsplit(runs,",")[[1]]) {  
 ...
 # data.allFaces$Packets <- data.allFaces$Packets /100
  data.allFaces$Packets = factor(data.allFaces$Packets) 
  ...
  #   g <- g + geom_point (aes(y=Packets.sum, color=Type), size=1)
   #  g <- g + scale_y_continuous (trans="log", labels = round)
    ...
    #g <- g + ggtitle ("Packets")
   # g <- g + scale_y_continuous(breaks=seq(0, 10, 0.25))
  ...
  nodes.evil = levels(data.allFaces$Node)[ grep ("^gw-", levels(data.allFaces$Node)) ]
