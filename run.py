        ...
        for algorithm in self.algorithms:
            for topology in self.topology:
                for evil in self.evils:
                    for run in self.runs:
                        cmdline = ["./build/project", #here has to change for different scenarios "testbed and project"
                                   #"./waf --run=tesbed --vis",
...
class CachehitRatioAlgorithms (SideChannelAttack):
    ...
    def postprocess (self):
        ...
        for algorithm in self.algorithms:
            for topology in self.topology:
                for evil in self.evils:
                    cmdline = ["./graphs/cachehitratio-preprocess.R",
                               algorithm,
                               topology,
                               str(evil),
                               ",".join([str(run) for run in self.runs]),
...
try:
    ...
    smallTree = SideChannelAttack (name="attack-small-tree",
                                    algorithms = ["Lru", "Fifo","Lfu", "Freshness","Probability", "Random"],
                                    topology = ["small-tree"],
                                    #evils = [1],
                                    evils = [1,2],
    ...
    testbed = SideChannelAttack (name="attack-testbed",
                              algorithms = ["Lru", "Fifo","Lfu", "Freshness","Probability", "Random"],
                              topology = ["testbed"],
                              evils = [1],
                              good  = 0, # number of client nodes minus number of evil nodes
                              runs = range(1,11), 
                              folder = "attack_testbed",
                              producer = "gw",
                              defaultRtt = "330ms")
    testbed.run ()
    ...
    CachehitRatioAlgorithms (name="ratios-testbed", scenario=testbed).run ()
    ...
    AttackersFigures (name="attacker-testbed", scenario=testbed).run ()
