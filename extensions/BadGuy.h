...
protected:
  ...
  std::string m_spy_leafname;  // the name of the good guys to spy on
  uint32_t m_spy_leafcount;    // the number of good guys to spy on
  uint32_t m_spy_bruteforce;    // if 1, do brute force attack on all good guys..
  std::vector<uint32_t> m_spy_list;  // random list of guys to attack
  std::vector<uint32_t> m_repeat_list; // repeat list
  uint32_t m_interest_repeat; // number of times to repeat the same interest packet
  Ptr<UniformRandomVariable> rand;  /// rand generator...
