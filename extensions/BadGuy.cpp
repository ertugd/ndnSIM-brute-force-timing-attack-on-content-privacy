...
BadGuy::OnData(shared_ptr<const Data> data)
{
	...
	//std::cout << "Signature and signature info: " << sig.getSignatureInfo() << "\n";
...
BadGuy::SendPacket()
{
  ...
  if (m_spy_bruteforce ==1) {
	  ...
	  if (!m_repeat_list.empty()) {  // if we have interestes to repeat, send them first...
		  leaf_number = m_repeat_list.back();
		  ...
		  for (uint32_t i=0; i < m_interest_repeat; i++)
			  m_repeat_list.push_back(leaf_number);
	  } else if (m_spy_list.empty())  // if attack lits is empty, give up
			  return;
	  nameWithSequence = make_shared<Name>(m_spy_prefix+"/"+m_spy_leafname+"-"+std::to_string(leaf_number));
  }
  //if (m_spy_seq > 0)
